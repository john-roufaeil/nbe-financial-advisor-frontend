      ...
      {data.map((d, i) => {
        ...
        return (
          <button
            ...
            className={`group flex cursor-pointer items-center gap-3 rounded-lg p-1 text-start transition-opacity duration-200 ${isDimmed ? "opacity-40" : "opacity-100"}`}
          >
            <span className="w-20 shrink-0 truncate text-sm font-medium">{d.name}</span>
            <span className="bg-base-200 h-5 min-w-0 flex-1 overflow-hidden rounded-full">
              <span
