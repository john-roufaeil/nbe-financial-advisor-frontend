...
Two compose files live in this repo and bring up **everything** — not just
the frontend — assuming `nbe-financial-advisor-backend` and
`nbe-financial-advisor-ai-service` are checked out as sibling directories
next to this one:

```
some-parent-dir/
├── nbe-financial-advisor-frontend/   (this repo — run docker compose from here)
...
Both need a real `.env` in each of the three app repos (this one,
`../nbe-financial-advisor-backend/.env`, `../nbe-financial-advisor-ai-service/.env`)
— copy each repo's `.env.example` first if you don't have one.
...
Set `SEED_MOCK_BANK_CUSTOMER_EMAIL=you@example.com` (e.g. in
`nbe-financial-advisor-backend/.env`) to also register a mock bank customer
with starter transaction history on startup, for exercising the real
"Connect bank" OTP flow against that inbox — leave it unset to skip.
...
docker compose -f docker-compose.prod.yml --env-file ../nbe-financial-advisor-backend/deploy/.env up -d --build
docker compose -f docker-compose.prod.yml --env-file ../nbe-financial-advisor-backend/deploy/.env logs -f <service>
docker compose -f docker-compose.prod.yml --env-file ../nbe-financial-advisor-backend/deploy/.env up -d --build <service>   # redeploy just one
...
There are `.env` files in every app repo plus one more in
`nbe-financial-advisor-backend/deploy/`, and they serve two genuinely
different purposes:

| File                                        | Loaded by                                                                                                                          | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nbe-financial-advisor-backend/.env`        | `env_file:` on `backend`/`celery-worker`/`backend-events` in both stacks                                                           | Real app config for the Django containers — `DJANGO_SECRET_KEY`, DB creds, feature toggles, `AI_SERVICE_TOKEN`, etc. Authoritative; the compose file's path to it doesn't change with where compose itself runs from.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `nbe-financial-advisor-backend/deploy/.env` | **Not `env_file:`-loaded anywhere.** Pass explicitly via `--env-file` when running `docker-compose.prod.yml` (see Commands above). | Docker Compose auto-loads a plain `.env` from the compose project directory to resolve any `${VAR}` in the YAML that isn't backed by `env_file:` — `docker-compose.prod.yml` has several of these (postgres passwords, `SEAWEED_SECRET_KEY`, `MOCK_BANK_JWT_SECRET`, `AI_SERVICE_TOKEN`'s override on `backend`'s own `environment:` block, etc). Since the compose files now live in this repo instead of `backend/deploy/`, that auto-load no longer finds this file on its own — without `--env-file`, those values silently fall back to placeholder defaults or blank. This file is almost identical to `backend/.env` (one extra key, `MOCK_BANK_OAUTH_PUBLIC_URL`) since it exists purely to serve this substitution role, not as a second independent config. |
...
the same dev-safe values already in `backend/.env`, and `mock-bank-oauth`/
...
instead (see the table in `docker-compose.dev.yml`), and the SSE route just
runs on `backend` alongside everything else — fine at dev traffic levels,
which is why isolating it is a prod-only concern.
